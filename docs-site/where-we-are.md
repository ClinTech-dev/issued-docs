...
Eight AI agents are working end-to-end, plus one deterministic monitoring engine:
...
- The **Note Synthesis Agent** reads a clinician's draft note in the context of the full admission — prior notes, active issues, outstanding tasks — and proposes a unified set of changes: new issues to add, existing issues to update, tasks to create, and a clean rewrite of the note for the clinician to review before submitting. It runs across multiple note types: medical admission, post-take round, daily review, consultant round, and ad-hoc review, with the rewrite style tuned to each.
- The **Linkage Agent** handles inbound clinical fragments that are not full notes — a result pasted into a scratchpad, a snippet forwarded from Teams, an observation arriving from another system. It matches the fragment to the issues it relates to and proposes structured linkages, new issues, or plan-review flags for the clinician to review.
...
- **Stale-issue monitoring** — a deterministic rule engine (parallel in design to the Patient-of-Concern Monitor) that flags issues whose plans have not been updated within a clinically expected window. This absorbs one of the jobs originally scoped to a standalone Gap Detection Agent.
