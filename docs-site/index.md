...
Issued is a clinical text editor built for inpatient ward teams. Think of it like Notion for the ward — a single place where clinicians write progress notes, manage patient issues, and prepare handover, all from a laptop on the ward. Each patient has a live, structured record: their active issues, linked results, and a running clinical narrative. The ward team works together inside it, so everyone sees the same picture, in real time.
...
```mermaid
flowchart TD
    A[Clinician writes in Issued] --> B[IssuedOS agents analyse the text]
